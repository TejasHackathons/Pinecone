...
const predictHurricaneMaxWind = async (month, latitude, longitude) => {
  const hurricaneModel = await tf.loadLayersModel(
    "https://raw.githubusercontent.com/TejasHackathons/Pinecone/main/backend/mlmodels/hurricaneModel.json"
  );
...
predictHurricaneMaxWind(1, 1, 1).then((wind) => console.log("f" + wind));
